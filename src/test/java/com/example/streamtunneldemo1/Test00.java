...
/**
 * StreamTunnel 性能测试
 * 测试 StreamRecordPack 的写入速度
 * <p>
 * flush batchSize 和 每次 flush 花费时间的关系 : 大致成正比 ，只有微调的空间
 * 3000    ====   100 ms
 * 10000   ====   271
 *
 *
...
/**
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 9, 0)  ===    29761 条/秒    flush time   270
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 0, 0)  ===    33333 条/秒    flush time   230
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 0, 4)  ===    30864 条/秒    flush time   257
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 5, 2)  ===    36231 条/秒    flush time
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 6, 2)  ===     条/秒         flush time  276
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_SNAPPY, 4, 2)  ===     条/秒         flush time  241
 *
 *
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_RAW, 5, 2)  ===     条/秒    flush time     xxx  flush 太慢了
 * batchSize   10000  ,  CompressOption.CompressAlgorithm.ODPS_ZLIB, 5, 2)  ===     条/秒    flush time    xxx  flush 太慢了
 *
 */
@Slf4j
@SpringBootTest
...
//    private HuyaListener listener;
    ...
    public void t1() throws Exception {
        ...
        TableTunnel.StreamUploadSession session = sessionManager.getSession(odpsConfiguration.getHuyaLiveLogTableName() + "&" + "20210301");
        ...
        for (int i = 0; i < 10000L * 500; i++) {
            pack.append(record);
            if (pack.getRecordCount() >= odpsConfiguration.getBatchSize()) {
                long t1 = System.currentTimeMillis();
                pack.flush();
                flushCount++;
                totalFlushTime += (System.currentTimeMillis() - t1);
                log.info("flushCount = {} , totalFlushTime = {}", flushCount, totalFlushTime);
            }
        }
